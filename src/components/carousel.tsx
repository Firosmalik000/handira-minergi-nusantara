	...
	return (
		<div className="perspective-[1200px] transform-style-preserve-3d">
			<motion.li
				...
				className={`relative w-[500px] h-[400px] rounded-2xl shadow-lg overflow-hidden transition-all duration-500  ${color} cursor-pointer group ${current !== index ? "scale-[0.96] rotate-x-[8deg]" : "scale-100"}`}
				style={{
			...
			>
				...
				<div className="relative z-10 flex flex-col items-start justify-center h-full px-6 text-center">
					...
					<div>
						<h2 className="text-start text-xl md:text-5xl lg:text-5xl font-bold font-playfair text-orange-500 group-hover:text-white">{title}</h2>
...
export function Carousel({ slides }: CarouselProps) {
	...
	return (
		<div className="relative w-full max-w-[90vw] md:w-[70vmin] md:h-[400px] mx-auto " aria-labelledby={`carousel-heading-${id}`}>
			<ul
				className="flex transition-transform duration-1000 ease-in-out mb-2"
				style={{
					width: `${slides.length * 55}%`,
